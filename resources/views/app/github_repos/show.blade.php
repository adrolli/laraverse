<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            @lang('crud.github_repos.show_title')
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <x-partials.card>
                <x-slot name="title">
                    <a href="{{ route('github-repos.index') }}" class="mr-4"
                ...
                <div class="mt-4 px-4">
                    <div class="mb-4">
                        ...
                        <span>{{ $githubRepo->title ?? '-' }}</span>
                    </div>
                    <div class="mb-4">
                        <h5 class="font-medium text-gray-700">
                            @lang('crud.github_repos.inputs.slug')
                        </h5>
                        <span>{{ $githubRepo->slug ?? '-' }}</span>
                    </div>
                    <div class="mb-4">
                        <h5 class="font-medium text-gray-700">
                            @lang('crud.github_repos.inputs.data')
                        </h5>
                        <pre>{{ json_encode($githubRepo->data) ?? '-' }}</pre>
                    </div>
                    <div class="mb-4">
                        <h5 class="font-medium text-gray-700">
                ...
                <div class="mt-10">
                    <a href="{{ route('github-repos.index') }}" class="button">
                        <i class="mr-1 icon ion-md-return-left"></i>
                        @lang('crud.common.back')
                    </a>

                    @can('create', App\Models\GithubRepo::class)
                    <a href="{{ route('github-repos.create') }}" class="button">
